...
export class OnboardingController {
  ...
  async addOrganizations(@UploadedFile() file: Express.Multer.File) {
    let list = [];
    try {
      ...
      readable
        ...
        .on("end", async () => {
          list = await this.onboardingService.addOrganizations(jsonData);
  ...
  async addBidvestUsers(
    ...
    try {
      ...
      readable
        ...
        .on("end", async () => {
          list = await this.userManager.addBidvestUsers(jsonData, divisionId);
  ...
  async addCoachUsers(
    ...
    try {
      ...
      readable
        ...
        .on("end", async () => {
          list = await this.userManager.addCoachUsers(
            jsonData,
          );
          Logger.debug(`${mm} Coaches uploaded successfully`);
        })
        .on("error", (error) => {
          Logger.error(`${mm} Error parsing CSV:`, error);
          throw error;
